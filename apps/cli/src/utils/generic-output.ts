...
export class AnalysisTextFormatter implements GenericOutputFormatter {
  format(data: any, context?: any): string {
    ...
    // --- TAMBAHAN: Metadata Git (Checklist: Dirty working tree & branch) ---
    ...
    // --- Perbaikan: Truncate & Duplication ---
    // Kita batasi hanya 5 (truncate) seperti permintaan owner
    ...
    // --- PERBAIKAN: Logic High Churn (Jangan tampilkan yang 0%) ---
    ...
    if (data.architectureSignals && data.architectureSignals.length > 0) {
      ...
      data.architectureSignals.forEach((signal: string) => {
        sections.push(`${chalk.gray('•')} ${chalk.yellow(signal)}`); // Pakai dot bullet biar lebih clean
      });
    }
